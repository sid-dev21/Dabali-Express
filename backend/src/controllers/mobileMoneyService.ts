...
/**
 * Interface for payment responses
 */
interface PaymentResponse {
  ...
  message?: string;
  paymentUrl?: string; // URL where the user should be redirected
}
...
export const initiateOrangeMoneyPayment = async (
  ...
  try {
    ...
    const response = await axios.post(
      `${ORANGE_MONEY_CONFIG.BASE_URL}/webpayment`,
      payload,
      {
        headers: {
 ...
 */
export const verifyOrangeMoneyPayment = async (
  transactionId: string
): Promise<PaymentResponse> => {
  try {
    const response = await axios.get(
      `${ORANGE_MONEY_CONFIG.BASE_URL}/transaction/${transactionId}`,
      {
        headers: {
          'Authorization': `Bearer ${ORANGE_MONEY_CONFIG.API_KEY}`,
        },
 ...
 */
export const initiateMoovMoneyPayment = async (
  amount: number,
  phone: string,
  reference: string,
  description: string
  ...
  try {
    ...
    const authResponse = await axios.post(
      `${MOOV_MONEY_CONFIG.BASE_URL}/auth/token`,
      {
    ...
    const response = await axios.post(
      `${MOOV_MONEY_CONFIG.BASE_URL}/payments/initiate`,
      payload,
      {
        headers: {
 ...
 */
export const verifyMoovMoneyPayment = async (
  transactionId: string
): Promise<PaymentResponse> => {
  try {
    // Authentication
    ...
    // Verification
    const response = await axios.get(
      `${MOOV_MONEY_CONFIG.BASE_URL}/payments/${transactionId}`,
      {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
