...
describe('Canteen Manager API - JavaScript Tests', () => {
  ...
  describe('Authentication Setup', () => {
    test('Should create school admin and login', async () => {
      // Create school first (direct DB insertion)
  ...
  describe('Canteen Manager Creation', () => {
    ...
    test('Should reject non-gmail emails', async () => {
      const response = await request(app)
        .post('/api/users/canteen-managers')
        .set('Authorization', `Bearer ${schoolAdminToken}`)
        .send({
          first_name: 'Traoré',
          last_name: 'Moussa',
          email: 'moussa.traore@yahoo.com', // Non-gmail email
          phone: '+22670333333',
          school_id: schoolId
        });
    ...
    test('Should accept gmail emails', async () => {
      const response = await request(app)
        .post('/api/users/canteen-managers')
        .set('Authorization', `Bearer ${schoolAdminToken}`)
        .send({
          first_name: 'Traoré',
          last_name: 'Moussa',
          email: 'moussa.traore@gmail.com', // Gmail email
          phone: '+22670333333',
          school_id: schoolId
        });
  ...
  describe('Canteen Manager Management', () => {
    ...
    beforeEach(async () => {
      ...
      managerId = response.body.data.user.id;
    });
