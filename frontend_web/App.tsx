...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Click outside handler pour la recherche
  ...
  const searchResults = useMemo(() => {
    if (!globalSearch || globalSearch.length < 2) return { students: [], schools: [], payments: [] };
    
    const term = globalSearch.toLowerCase();
    const allStudents = mockApi.getStudents(currentUser?.schoolId);
    const allSchools = mockApi.getSchools();
    const allPayments = mockApi.getPayments(currentUser?.schoolId);

    return {
      students: allStudents.filter(s => `${s.firstName} ${s.lastName}`.toLowerCase().includes(term)).slice(0, 5),
      schools: currentUser?.role === UserRole.SUPER_ADMIN 
        ? allSchools.filter(s => s.name.toLowerCase().includes(term)).slice(0, 3) 
        : [],
      payments: allPayments.filter(p => p.studentName.toLowerCase().includes(term)).slice(0, 3)
    };
  }, [globalSearch, currentUser]);

  const hasResults = searchResults.students.length > 0 || searchResults.schools.length > 0 || searchResults.payments.length > 0;
